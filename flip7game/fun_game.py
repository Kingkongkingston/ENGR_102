...
class CardTile(ttk.Frame):
    def __init__(self, master, card):
        ...
        self.card = card
        self.canvas = tk.Canvas(self, width=CARD_WIDTH,
                                height=CARD_HEIGHT, highlightthickness=0)
...
class ScrollFrame(ttk.Frame):
    def __init__(self, master):
        ...
        self.h = ttk.Scrollbar(self, orient='horizontal',
        ...
        self.win = self.canvas.create_window(
...
class Flip7GUI(tk.Tk):
    ...
    def _build_left(self):
        ...
        fr.pack(pady=6)
        ttk.Button(fr, text="Add", command=self._add_player_dialog).pack(
            side='left', padx=4)
        ttk.Button(fr, text="Remove", command=self._remove_selected).pack(
            side='left', padx=4)
        ttk.Button(fr, text="New Game", command=self._new_game).pack(
            side='left', padx=4)

        tk.Label(self.left, text="Scores", bg=COLOR_LEFT, fg=COLOR_PANEL_TITLE, font=(
            "Helvetica", 12, "bold")).pack(anchor='nw', padx=12, pady=(10, 4))
        self.scorebox = tk.Text(self.left, height=6,
                                width=18, bg=COLOR_LEFT, bd=0)
    ...
    def _build_right(self):
        ...
        self.btn_hit = tk.Button(
            fr, text="HIT", bg="#1ABC9C", fg="white", width=14, command=self._hit)
        self.btn_stay = tk.Button(
            fr, text="STAY", bg="#F39C12", fg="white", width=14, command=self._stay)
        self.btn_start = tk.Button(
            fr, text="Start Round", bg="#3498DB", fg="white", width=14, command=self._start_round)
        ...
        ttk.Separator(self.right, orient='horizontal').pack(
        ...
        self.status = tk.Label(
            self.right, text="Status: Waiting", bg=COLOR_RIGHT, anchor='w')
    ...
    def _prompt_initial_players(self):
        n = simpledialog.askinteger(
            "Players", "Number of players (2-8):", minvalue=2, maxvalue=12)
        if not n:
            n = 2
        for i in range(n):
    ...
    def _render_players(self):
        ...
        for i, p in enumerate(self.players):
            r = i//cols
            c = i % cols
            f = tk.Frame(self.table_inner, bg=COLOR_CENTER,
                         relief='raised', bd=2)
            ...
            tk.Label(f, text=f"Total: {self.scores.get(p.name, 0)}", bg=COLOR_CENTER).pack(
                anchor='nw', padx=6)

            # number tiles
            nf = tk.Frame(f, bg=COLOR_CENTER)
            nf.pack(fill='x', padx=6, pady=(8, 4))
            tk.Label(nf, text="Numbers:", bg=COLOR_CENTER).pack(side='left')
            num_tiles = tk.Frame(nf, bg=COLOR_CENTER)
            num_tiles.pack(side='left', padx=6)

            mf = tk.Frame(f, bg=COLOR_CENTER)
            mf.pack(fill='x', padx=6, pady=4)
            tk.Label(mf, text="Modifiers:", bg=COLOR_CENTER).pack(side='left')
            mod_tiles = tk.Frame(mf, bg=COLOR_CENTER)
            mod_tiles.pack(side='left', padx=6)

            af = tk.Frame(f, bg=COLOR_CENTER)
            af.pack(fill='x', padx=6, pady=4)
            tk.Label(af, text="Actions:", bg=COLOR_CENTER).pack(side='left')
            act_tiles = tk.Frame(af, bg=COLOR_CENTER)
            act_tiles.pack(side='left', padx=6)

            status = tk.Label(f, text="Status: Ready",
                              bg=COLOR_CENTER, anchor='w', relief='groove')
    ...
    def _refresh_tiles(self):
        for pf in self.player_frames:
            ...
            for v in p.number_cards:
                ...
                t.canvas.itemconfigure(
                    "bg", fill=COLOR_NUMBER, outline=COLOR_TILE_BORDER)
            ...
            for mod in p.modifiers:
                ...
                t.canvas.itemconfigure(
                    "bg", fill=COLOR_MODIFIER, outline=COLOR_TILE_BORDER)
                t.pack(side='left', padx=6, pady=6)
            ...
            for act in p.action_cards:
                ...
                t.canvas.itemconfigure(
                    "bg", fill=COLOR_ACTION, outline=COLOR_TILE_BORDER)
                t.pack(side='left', padx=6, pady=6)
        ...
        for p in self.players:
            ...
            if c.type == 'action':
                if c.value == 'flip_three':
                    self._log(
                        f"Flip Three during deal -> resolving for {p.name}")
                    self._flip_three(p)
                elif c.value == 'freeze':
                    p.stayed = True
                    p.active = False
    ...
    def _hit(self):
        ...
        if c.type == 'action' and c.value == 'flip_three':
            ...
            if res[0] == 'second_chance_already':
                others = [
                    pl for pl in self.players if pl is not p and pl.active and 'Second Chance' not in pl.action_cards]
                if others:
        ...
        def process(i):
            if i >= len(drawn) or player.busted:
                if 'Freeze' in player.action_cards:
                    ...
                    self._log(
                        f"{player.name} freeze resolved after Flip Three.")
                self._refresh_players_after_change()
                return
            ...
            if res[0] == 'bust':
                ...
                self._log(
                    f"{player.name} uses Second Chance during Flip Three.")
